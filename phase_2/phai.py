...
data_rdd = (
    ...
    .csv("file:/home/phai597/p2_Team1_Data.csv")
)
...
)  # Creates a new dataframe that is our normal dataframe, but with a new column 'time' that gives only time, not date
...
only_time = only_time.createOrReplaceTempView("data_2") #SQL-friendly view of our new table with the 'time' column
...
highest_time_of_sales_by_country = spark.sql(
    "SELECT time, country FROM data_2 ORDER BY country ASC"
